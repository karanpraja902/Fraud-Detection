...
sys.path.append('src')
from data.preprocess import run_preprocessing, load_data, normalize_features, handle_imbalance
...
def data_validation_task(data_info: dict, previous_training_stats_path: str = "training_stats.json"):
    """
    ...
    if Path(previous_training_stats_path).exists():
        try:
            ...
            baseline_fraud_rate = previous_stats.get("fraud_rate", data_info["fraud_rate"])

            # Calculate drift
            fraud_rate_drift = abs(fraud_rate - baseline_fraud_rate) / baseline_fraud_rate
            if fraud_rate_drift > VALIDATION_CONFIG["max_class_imbalance_ratio"]:
                ...
                validation_results["quality_metrics"]["fraud_rate_drift"] = fraud_rate_drift
    ...
    print(f"✅ Data validation completed. Valid: {validation_results['is_valid']}, Warnings: {len(validation_results['warnings'])}, Errors: {len(validation_results['errors'])}")
...
def data_preprocessing_task(validation_results: dict, data_info: dict):
    ...
    preprocessing_stats = {
        ...
        "feature_stats": {
            ...
            "fraud_transaction_mean": X_train.groupby('Class')['Amount'].mean().to_dict()
        }
    }
...
def model_training_task(preprocessing_results: dict):
    ...
    evaluator = ModelEvaluator("models/latest_model.pkl", "fraud_detection_continuous_training")
    ...
    meets_criteria = (
        auc_score >= TRAINING_CONFIG["min_auc_score"] and
        recall_fraud >= TRAINING_CONFIG["min_recall_fraud"]
    )
...
def model_validation_task(training_results: dict):
    ...
    if metrics.get("recall_fraud", 0) < 0.8:  # Must catch at least 80% of fraud
        ...
        validation_results["recommendations"].append("Improve fraud recall - consider different model or resampling")
    else:
    ...
    # Threshold analysis
    optimal_threshold = find_optimal_threshold_for_business(training_results["evaluation_results"])
    ...
    validation_results["model_ready_for_production"] = (
        validation_results["model_ready_for_production"] and
        validation_results["validation_checks"].get("fraud_recall") == "PASS"
    )

    print(f"Model ready for production: {validation_results['model_ready_for_production']}")
...
def model_registration_task(training_results: dict, validation_results: dict):
    ...
    with mlflow.start_run():
        ...
        mlflow.log_params({
            ...
            "meets_production_criteria": validation_results["model_ready_for_production"]
        })
...
def update_training_stats_task(data_info: dict, preprocessing_results: dict):
    ...
    if Path(stats_path).exists():
        try:
            ...
            existing_stats["training_history"] = existing_stats.get("training_history", [])
...
@flow(name="Continuous Training Pipeline", description="Automated ML training pipeline with validation and registration")
def continuous_training_pipeline(
    ...
    pipeline_results = {
        ...
        "steps_completed": [
            "data_ingestion", "data_validation", "data_preprocessing",
            "model_training", "model_validation", "model_registration",
            "stats_update"
        ],
